...
public class ApplicationP3{
	...
	public static void main(String[] args) {		
		...
		CurrencyConverterUsingXML.displayDatabase();
		startMenu();	
	...
	public static void performAction() {
		...
		case "d":
			...
			break;
		case "i":
			CurrencyConverterUsingXML.XML.initializeFile();
			CurrencyConverterUsingXML.displayDatabase();
			nextOption();
			break;
		default:
	...
	public static void currencyUserInterface(String question) {
		...
		if(CurrencyConverterUsingXML.currencyExists(currencyToConvert)) {
	...
	public static void conversionAmountUserInterface() {
		...
		else System.out.println("" + conversionAmount + " "+ convertFrom.toUpperCase() + " is equivalent to " + CurrencyConverterUsingXML.convert(conversionAmount, convertFrom, convertTo) + " " + convertTo.toUpperCase()+"\n");
	}
	...
	public static void addCurrencyUserInterface() {
		...
		try {
			...
			if(exchangeRateValue.compareTo(zero)==1) {
				CurrencyConverterUsingXML.addCurrency(currency, exchangeRateValue);
	...
	public static void updateCurrencyUserInterface() {
		...
		try {
			...
			if(exchangeRateValue.compareTo(zero) ==1 && !currency.equals("usd")) {
				CurrencyConverterUsingXML.updateCurrency(currency, exchangeRateValue);
				System.out.println("\nYour request was successfully processed!\n");
				CurrencyConverterUsingXML.displayDatabase();
				nextOption();
			}
	...
	public static void removeCurrencyUserInterface() {
		...
		if(currency.equals(convertFrom)||currency.equals("usd")) {
			...
			try {
				CurrencyConverterUsingXML.removeCurrency(currency);
